...
**本项目是一个帮助你在 AI 时代发现个人优势的 Skill，通过综合心理、认知、人格、职业发展及经济等多门学科，以对话方式分析|发掘个人在 AI 时代下的特色及优势，专注未来发展方向**
...
PCAE 同时兼容 **Claude Code** 和 **OpenAI Codex CLI**——两者都使用 Skill 机制，目录不同。
...
Codex CLI 的 Skill 目录是 `~/.agents/skills/`（个人级）或仓库内的 `.agents/skills/`（团队共享级）。
...
- **显式调用** — 输入 `$pcae`（或斜杠菜单中选择），明确告诉 Codex 使用此 Skill
- **隐式调用** — 直接描述需求（如"帮我分析职业优势"），Codex 会根据 SKILL.md 中的 description 自动选用 PCAE

> Codex 使用 *progressive disclosure*：启动时仅加载 Skill 的 name + description（节省上下文），只在决定使用时才读取完整 SKILL.md 和 references。这与 PCAE 的"按需加载"设计天然契合。
...
Codex CLI's Skill directory is `~/.agents/skills/` (personal) or `.agents/skills/` inside a repository (team-shared).
...
- **Explicit** — Type `$pcae` (or pick from the slash menu) to tell Codex to use this Skill
- **Implicit** — Describe your need naturally (e.g. "help me analyze my career advantages") and Codex picks PCAE based on the SKILL.md description

> Codex uses *progressive disclosure*: at startup, it only loads each Skill's name + description (saving context), and reads the full SKILL.md and references only when it decides to use the Skill. This pairs naturally with PCAE's on-demand loading design.
